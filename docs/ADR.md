...
### ADR-008: 렌더 관련 의존성 버전은 실측으로 고정한다 (numpy==2.2.6)
**결정**: `projects/square_assembly/requirements.txt`에 `numpy==2.2.6`을 고정한다. 렌더 품질이 곧
데이터가 되는 작업(개입/텔레옵 수집, 평가)은 이 requirements로 만든 환경이면 도커 컨테이너든
호스트 네이티브(pororo conda env `sig`)든 어디서 해도 된다 — 도커를 우선한다(재현성, ADR-007).

**이유**: 2026-09-18 numpy 2.4.6 + mujoco 3.2.3에서 MuJoCo 오프스크린 렌더가 초기화되지 않은
메모리를 돌려주는 문제를 확정했다(네이티브·컨테이너 모두 재현, 2.2.6으로만 바꾸면 모두 정상 —
DOCKER.md §4). 이틀간 도커·GPU 부하·X11 등을 의심하며 잃은 시간이 크다. 렌더 경로에 닿는
패키지(numpy, mujoco, robosuite, PyOpenGL)는 상한 없는 `>=`로 두지 않고 실측된 조합으로 고정한다.

**트레이드오프**: 보안/버그 수정 업데이트를 자동으로 못 받는다. 올릴 때는 DOCKER.md §4의
20-step 렌더 점검을 통과하는지 확인한 뒤 핀을 바꾼다.
