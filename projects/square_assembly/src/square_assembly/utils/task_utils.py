...
def _burn_first_episode(env, n_steps=30):
    ...
    (2026-09-18 실측: 컨테이너는 numpy 버전과 무관하게 매번 재현, 두 번째 reset부터는 항상 정상.
    호스트 네이티브는 재현 안 됨 — DOCKER.md §4). 그래서 image env는 만들자마자 reset+n_steps를
    한 번 버려서 호출부가 받는 첫 에피소드부터 정상이게 한다. 네이티브에선 불필요하지만 0.1초라
    항상 한다. eval_seed는 호출부가 이 뒤에 잡으므로 영향 없다."""
    env.reset()
